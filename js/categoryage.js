...
let catagemargin = {top: 20, right: 50, bottom: 50, left: 30}
    , cateagewidth = 530 // Use the window's linewidth
    , catageheight = 400; // Use the window's lineheight

/////////////////////////////
// type
agetypeExecute = function () {

    let catrate_svg = d3.select("#agetyperate").append("svg")
        .attr("width", cateagewidth + catagemargin.left + catagemargin.right)
        .attr("height", catageheight + catagemargin.top + catagemargin.bottom);

    let catratesvg = catrate_svg.append("g")
        .attr("transform", "translate(" + catagemargin.left + "," + catagemargin.top + ")");

    var x0 = d3.scaleBand().range([0, cateagewidth]).padding(0.55);
    var x1 = d3.scaleBand();

    var typey = d3.scaleLinear()
        .rangeRound([catageheight, 0]);
    ...
    var color = d3.scaleOrdinal()
        .range(["steelblue", "darkorange", "lightblue", "orange"]);
    ...
    var innerColumns = {
        "column1" : ["20 and under", "21 to 24", "25 to 29", "30 and over"]
        // "column2" : ["Female"],
    };

    d3.csv("data/cleaned/ratebyagetype.csv").then(
        function(data) {
            ...
            catratesvg.append("g")
                .attr("class", "xaxis")
                .attr("transform", "translate(0," + catageheight + ")");
...
/////////////////////////////
// level
agelevelExecute = function () {

    let levelrate_svg = d3.select("#agelevelrate").append("svg")
        .attr("width", cateagewidth + catagemargin.left + catagemargin.right)
        .attr("height", catageheight + catagemargin.top + catagemargin.bottom);
...
//.range(["#98abc5", "#8a89a6", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]);
    ...
    d3.csv("data/cleaned/ratebyagelevel.csv").then(
        function(data) {
...
/////////////////////////////
// Year of Study
ageyearExecute = function () {

    let yearrate_svg = d3.select("#ageyearrate").append("svg")
        .attr("width", cateagewidth + catagemargin.left + catagemargin.right)
        .attr("height", catageheight + catagemargin.top + catagemargin.bottom);
    ...
    d3.csv("data/cleaned/ratebyageyear.csv").then(
        function(data) {
...
d3.select("#agetype-button").on("click", function () {
    d3.select("#agelevelrate svg").remove();
    d3.select("#ageyearrate svg").remove();
    agetypeExecute();
});

d3.select("#agelevel-button").on("click", function () {
    d3.select("#agetyperate svg").remove();
    d3.select("#ageyearrate svg").remove();
    agelevelExecute();
});

d3.select("#ageyear-button").on("click", function () {
    d3.select("#agelevelrate svg").remove();
    d3.select("#agetyperate svg").remove();
    ageyearExecute();
});
